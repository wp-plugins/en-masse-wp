...
/*
 Requirements: jQuery 1.5 or above
  ...
  Usage:
  jQuery(target).smartAutoComplete({options})
...
function addRow()
{
	count = 0;//jQuery("input[name='num_of_branches']").val();
...
function lookup(inputString) {
	...
	if(inputString.length == 0) {
			...
			jQuery.post("admin.php?page=enmasse_merchant&action=getuser&noheader=true", {queryString: ""+inputString+""}, function(data){
				if(data.length >0) {
  ...
  jQuery.fn.smartAutoComplete = function(){    
    ...
    var default_options = {
                            ...
                            filter: function(term, source){    
                              var context = this;
                              var options = jQuery(context).data('smart-autocomplete');
                              ...
                              else if(jQuery.type(source) === "string"){
                            ...
                            clearResults: function(){
                              ...
                              var type_ahead_field = jQuery(this.context).prev(".smart_autocomplete_type_ahead_field");
                              jQuery(this.context).css({ background: type_ahead_field.css("background") });
                            ...
                            setCurrentSelectionToContext: function(){
                                if(this.rawResults.length > 0 && this.currentSelection >= 0)
                                  jQuery(this.context).val(this.rawResults[(this.currentSelection)]);
                            },
    ...
    jQuery.event.special.keyIn = {
      ...
      _default: function(ev){
        ...
        setTimeout(function(){
          jQuery.when( filter.apply(options, [query, options.source]) ).done(function( results ){
            //do the trimming
            var trimmed_results = (options.maxResults > 0 ? results.splice(0, options.maxResults) : results);

            jQuery(context).trigger('resultsReady', [trimmed_results]);
          });
        }, options.delay);
      }
    };

    jQuery.event.special.resultsReady = {
      setup: function(){ return false },
      ...
      _default: function(ev){
        ...
        jQuery(context).smartAutoComplete().clearResults();
        ...
        if(results.length < 1){
          jQuery(context).trigger('noResults');
          return false
        }

        //call the results formatter function
        var formatted_results = jQuery.map(results, function(result){
        ...
        if(options.resultsContainer)
          jQuery(options.resultsContainer).append(formatted_results_html);

        //trigger results ready event
        jQuery(context).trigger('showResults', [results]);
      }             
    };

    jQuery.event.special.showResults = {
      setup: function(){ return false },

      _default: function(ev){    
        var context = ev.target;
        var options = jQuery(context).data("smart-autocomplete");
        var results_container = jQuery(options.resultsContainer);

        //event specific data
        var raw_results = ev.smartAutocompleteData.results; 

        //type ahead
        if(options.typeAhead && (raw_results[0].substr(0, jQuery(context).val().length) == jQuery(context).val()) ){
          var suggestion = raw_results[0]; //options.typeAheadExtractor(jQuery(context).val(), raw_results[0]); 
          
          //add new typeAhead field
          jQuery(context).before("<input class='smart_autocomplete_type_ahead_field' type='text' autocomplete='off' disabled='disabled' value='" + suggestion + "'/>");
          ...
          var typeAheadField = jQuery(context).prev("input");
          typeAheadField.css({
            position: "absolute",
            zIndex: 1,
            overflow: 'hidden',
            background: jQuery(context).css("background"),
            borderColor: 'transparent',
            width: jQuery(context).width(),
            color: 'silver'
          });
          ...
          if(results_container)
            jQuery(context).trigger('itemFocus', results_container.children()[options.currentSelection]);
        }
        ...
        if(results_container){
          if(options.alignResultsContainer){
            results_container.css({ 
                  position: "absolute",
                  top: function(){ return jQuery(context).offset().top + jQuery(context).height(); }, 
                  left: function(){ return jQuery(context).offset().left; }, 
                  width: function(){ return jQuery(context).width(); }, 
                  zIndex: 1000
            })
          }  
    ...
    jQuery.event.special.noResults = {
      setup: function(){ return false },

      _default: function(ev){    
        var context = ev.target;
        var options = jQuery(context).data("smart-autocomplete");
        var result_container = jQuery(options.resultsContainer);
    ...
    jQuery.event.special.itemSelect = { 
      setup: function(){ return false },
      ...
      _default: function(ev){    
        ...
        var selected_value = jQuery(selected_item).text() || jQuery(selected_item).val();
        //set it as the value of the autocomplete field
        jQuery(context).val(selected_value); 

        //set item selected property
        options.setItemSelected(true);

        //set number of current chars in field 
        options.originalCharCount = jQuery(context).val().length;
        
        //trigger lost focus
        jQuery(context).trigger('lostFocus');
      }
    };

    jQuery.event.special.itemFocus = {
      setup: function(){ return false },
      ...
      _default: function(ev){    
        ...
        jQuery(item).addClass("smart_autocomplete_highlight");
      }
    };

    jQuery.event.special.itemUnfocus = { 
      setup: function(){ return false },
      ...
      _default: function(ev){    
        ...
        jQuery(item).removeClass("smart_autocomplete_highlight");
      }
    }

    jQuery.event.special.lostFocus = {
      setup: function(){ return false },
      ...
      _default: function(ev){    
        ...
        if(options.resultsContainer)
          jQuery(options.resultsContainer).hide();
    ...
    return this.each(function(i) { 
      //set the options
      var options = jQuery.extend(default_options, jQuery(this).data("smart-autocomplete"), passed_options);
      //set the context
      options['context'] = this;

      //if a result container is not defined
      if(jQuery.type(options.resultsContainer) === 'undefined' ){
        //define the default result container if it is already not defined
        var default_container = jQuery("<ul class='smart_autocomplete_container' style='display:none'></ul>");
      ...
      jQuery(this).data("smart-autocomplete", options);

      // bind user events
      jQuery(this).keyup(function(ev){
        //get the options
        var options = jQuery(this).data("smart-autocomplete");
        ...
        if(ev.keyCode == '38'){
          ...
          if(options.resultsContainer){
            ...
            var result_suggestions = jQuery(options.resultsContainer).children();

            if(current_selection >= 0)
              jQuery(options.context).trigger('itemUnfocus', result_suggestions[current_selection] );
            ...
            jQuery(options.context).trigger('itemFocus', [ result_suggestions[current_selection] ] );
          }
        }
        ...
        //down arrow
          ...
          //trigger keyIn event on down key
            ...
            jQuery(options.context).trigger('keyIn', [jQuery(this).val()]); 
          }
        ...
        //right arrow & enter key
          ...
          var type_ahead_field = jQuery(options.context).prev('.smart_autocomplete_type_ahead_field');
          if(options.resultsContainer && jQuery(options.resultsContainer).is(':visible')){
            var current_selection = options.currentSelection;
            var result_suggestions = jQuery(options.resultsContainer).children();

            jQuery(options.context).trigger('itemSelect', [ result_suggestions[current_selection] ] );
          }
          else if(options.typeAhead && type_ahead_field.is(':visible'))
            jQuery(options.context).trigger('itemSelect', [ type_ahead_field ] );

          return false;
        }

        else {
         var current_char_count = jQuery(options.context).val().length;
         ...
         if(current_char_count >= options.minCharLimit){
          jQuery(options.context).trigger('keyIn', [jQuery(this).val()]); 
            ...
            if(options.autocompleteFocused){ 
              options.currentSelection = null;
              jQuery(options.context).trigger('lostFocus');
            }
         }
      ...
      jQuery(this).focus(function(){
        ...
        jQuery(this).closest("form").bind("keydown.block_for_smart_autocomplete", function(ev){
          var type_ahead_field = jQuery(options.context).prev('.smart_autocomplete_type_ahead_field');
          if(ev.keyCode == '13'){
            if(options.resultsContainer && jQuery(options.resultsContainer).is(':visible')){
              var current_selection = options.currentSelection;
      ...
      jQuery(document).bind("focusin click", function(ev){
        if(options.autocompleteFocused){ 
          var elemIsParent = jQuery.contains(options.resultsContainer[0], ev.target);
          if(ev.target == options.resultsContainer[0] || ev.target == options.context || elemIsParent) return

          jQuery(options.context).closest("form").unbind("keydown.block_for_smart_autocomplete");
          jQuery(options.context).trigger('lostFocus');
        }
      });

      //bind events to results container
      jQuery(options.resultsContainer).delegate(options.resultElement, 'mouseenter.smart_autocomplete', function(){
        var current_selection = options.currentSelection || 0;
        var result_suggestions = jQuery(options.resultsContainer).children();

        options['currentSelection'] = jQuery(this).prevAll().length;

        jQuery(options.context).trigger('itemFocus', [this] );
          
      });

      jQuery(options.resultsContainer).delegate(options.resultElement, 'mouseleave.smart_autocomplete', function(){
        jQuery(options.context).trigger('itemUnfocus', [this] );
      });

      jQuery(options.resultsContainer).delegate(options.resultElement, 'click.smart_autocomplete', function(){
        jQuery(options.context).trigger('itemSelect', [this]);
        return false
      });

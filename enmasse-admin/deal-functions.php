...
/*
 * @todo Set deals status
 */
function setDealStatus($status,$dstring) {
	global  $wpdb;
	$query = "UPDATE ".wp_enmasse_deal." SET status='".$status."' WHERE  id IN(".$dstring.")";
	return $wpdb->query($query);
}
/*
 * @todo check previous status of deals on changing deals status.
 */
function checkPrivousStatus($status,$ids) {
		global $wpdb; 
		$status = str_replace(' ','',$status);
		$previous = array('OnSales'=>'Upcoming','Confirmed'=>'On Sales','Closed'=>'On Sales');
		$status = $previous[$status];
		$query = "SELECT d.status as status FROM ".ENMASSE_DEAL." AS d WHERE d.id IN (".$ids.")";
		$deals = $wpdb->get_results($query);
		foreach ($deals as $deal)
		{
			if(strtolower($deal->status) !== strtolower($status))
			{
				return false;
			}
		} 
 ...
 * @todo get buyer mail
 */
function  get_buyer($dstring) {
	global $wpdb;
	$query = "SELECT d.name as deal_name,o.buyer_id,o.id as order_id FROM ".ENMASSE_ORDER." AS o 
								INNER JOIN ".ENMASSE_ORDER_ITEM.
								" AS i ON o.id = i.order_id INNER JOIN ".ENMASSE_DEAL." AS d ON d.id = i.unit_id". 
								" WHERE i.unit_id IN (".$dstring.")";	
	$buyerids = $wpdb->get_results($query);
	$buyers = array();
	foreach ($buyerids as $buyerid) {
		$user = get_userdata($buyerid->buyer_id);
		$buyerid->user_email = $user->user_email;
		$buyerid->user_nicename = $user->user_nicename;
		$buyers[] = $buyerid;
	}
...
/*
 * @todo send mail to related people when change deals status
 */
function do_send_mail($status,$dstring) {
	$enmasse_mail = new Enmasse_Mail();
	$buyers = get_buyer($dstring);
	if(strtolower($status) == "confirmed") {
		foreach ($buyers as $buyer) {
			$enmasse_mail->sendConfirmDealBuyermail($buyer->user_email, $buyer->order_id, $buyer->deal_name, $buyer->user_nicename, 'Enmasse', 'deliver@matamko.com');
		}
	}
	else if(strtolower($status) == "closed") {
		$enmasse_mail->sendVoidDealmail($buyer->user_email, $buyer->deal_name, $buyer->order_id, $buyer->deal_name, 'refund');
	}
}
